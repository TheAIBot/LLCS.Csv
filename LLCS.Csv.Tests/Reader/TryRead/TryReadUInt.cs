...
public sealed class TryReadNUInt
{
    private sealed class NUIntStorage : ICsvSerializer
    {
        ...
        public bool TryDeSerialize(CsvReader reader, ref ReadOnlySpanTokenizer<char> tokens) => reader.TryReadNUInt(ref tokens, out Value);
    }
    ...
    private sealed class NUIntStorageNumberStyleAndCulture<T> : ICsvSerializer where T : INumberStyleAndCulture, new()
    {
        ...
        {
            T value = new T();
            return reader.TryReadNUInt(ref tokens, value.NumberStyle, value.CultureInfo, out Value);
        }
    }
