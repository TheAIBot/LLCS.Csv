...
public sealed class TryReadShort
{
    private sealed class ShortStorage : ICsvSerializer
    {
        ...
        public bool TryDeSerialize(CsvReader reader, ref ReadOnlySpanTokenizer<char> tokens) => reader.TryReadShort(ref tokens, out Value);
    }
    ...
    private sealed class ShortStorageNumberStyleAndCulture<T> : ICsvSerializer where T : INumberStyleAndCulture, new()
    {
        ...
        {
            T value = new T();
            return reader.TryReadShort(ref tokens, value.NumberStyle, value.CultureInfo, out Value);
        }
    }
