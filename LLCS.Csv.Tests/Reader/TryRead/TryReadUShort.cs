...
public sealed class TryReadUShort
{
    private sealed class UShortStorage : ICsvSerializer
    {
        ...
        public bool TryDeSerialize(CsvReader reader, ref ReadOnlySpanTokenizer<char> tokens) => reader.TryReadUShort(ref tokens, out Value);
    }
    ...
    private sealed class UShortStorageNumberStyleAndCulture<T> : ICsvSerializer where T : INumberStyleAndCulture, new()
    {
        ...
        {
            T value = new T();
            return reader.TryReadUShort(ref tokens, value.NumberStyle, value.CultureInfo, out Value);
        }
    }
...
public sealed class TryReadByte
{
    private sealed class ByteStorage : ICsvSerializer
    {
        ...
        public bool TryDeSerialize(CsvReader reader, ref ReadOnlySpanTokenizer<char> tokens) => reader.TryReadByte(ref tokens, out Value);
    }
    ...
    private sealed class ByteStorageNumberStyleAndCulture<T> : ICsvSerializer where T : INumberStyleAndCulture, new()
    {
        ...
        {
            T value = new T();
            return reader.TryReadByte(ref tokens, value.NumberStyle, value.CultureInfo, out Value);
        }
    }
